...
<div class="hDiv">
  ...
  <table cellspacing="0" cellpadding="0" border="0" style="" class="flexme3">
	  ...
	  $total_price=0;
	  $total_payment=0;
	  foreach ($supplier_ledgers as $invoice){ 
			   $total_price= $total_price+$invoice['PosSupplierLedger']['totalprice'];
			   $total_payment=$total_payment+$invoice['PosSupplierLedger']['payamount'];
			  
	  ?>
	 	<tr id='<?php echo 'row_'.$invoice['PosSupplierLedger']['id'];?>'>
	 	
		<td align='left'><div style='width: 100px;' class='alistname'><?php 
		echo date("d/m/Y", strtotime($invoice['PosSupplierLedger']['created']));
		
		//echo $invoice['PosSupplierLedger']['created']; ?>&nbsp;</div></td>
		<td align='left'><div style='width: 200px;' class='alistname'>
		 	 	<?php echo $payment_mode[$invoice['PosSupplierLedger']['payment_mode']]; ?>
			
		</div></td>
		<td align='left'><div style='width: 150px;' class='alistname'>
			 
			<?php echo $invoice['PosSupplierLedger']['totalprice']; ?>
		</div></td>
		<td align='left'><div style='width: 100px;' class='alistname'><?php  echo $invoice['PosSupplierLedger']['payamount']; ?>&nbsp;</div></td>
   	</tr> 
<?php }  ?>
<?php 
   if($total_price >0){ ?>
