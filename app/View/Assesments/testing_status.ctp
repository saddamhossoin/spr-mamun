...
<?php echo $this->Form->input('AssesmentApproveNote.service_device_info_id',array('value'=>$id_re_assement,'div'=>false,'label'=>false,'type'=>'hidden' ));?>  
	...
	<div class="button_area">
		<?php	echo $this->Form->button('Save',array( 'class'=>'submit', 'id'=>'btn_AssesmentApproveNotes_TestComplete'));?>
...
<script>
jQuery(function($){ 
 	 $('#btn_AssesmentApproveNotes_TestComplete').click(function(e){
		 e.preventDefault();
 		//========================== Validation Check ========
 			$('.overlaydiv').fadeIn();
			$('.ajax-save-message').hide().fadeOut(); 
 			...
 			$.ajax({
			type: "POST",
			url:siteurl+"Assesments/testing_status/",
			data:  $("#AssesmentApproveNoteTestingStatusForm").serialize(),
			success: function(response){
			 
				if(response == 1){
					location.reload();
				}else{
					 $.alert.open("Test Complete job is not done.");
				}
 			 //  
 		 		}
 			});
