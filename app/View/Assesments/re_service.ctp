...
<?php echo $this->Form->input('AssesmentApproveNote.service_device_info_id',array('value'=>$id_re_assement,'div'=>false,'label'=>false,'type'=>'hidden' ));?>  
	...
	<div class="button_area">
		<?php	echo $this->Form->button('Save',array( 'class'=>'submit', 'id'=>'btn_AssesmentApproveNotes_Re_service'));?>
...
<script>
jQuery(function($){ 
 	 $('#btn_AssesmentApproveNotes_Re_service').click(function(e){
		 e.preventDefault();
 		//========================== Validation Check ========
 			$('.overlaydiv').fadeIn();
			...
			success: function(response){
				if(response == 1){
					location.reload();
					 ...
					 $.alert.open("Re-Service job is not done.");
				}
 			 //  
 		 		}
 			});
