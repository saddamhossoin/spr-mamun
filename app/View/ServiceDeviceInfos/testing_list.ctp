...
<?php
    $purchaseDate = '';
	$i = 0;
	foreach ($testing_lists as $testing_list):
	...
	//pr($assessment_list);
		...
		$dataDate = date('d / m / Y',strtotime($testing_list['ServiceDeviceInfo']['created']));
			if($purchaseDate == "" ||  $dataDate!= $purchaseDate){
				echo "<tr><td colspan = '10' class = 'listDateHeading'>".$dataDate."</td></tr>";
				$purchaseDate = $dataDate;
			}
	?>
	<tr id='<?php echo 'row_'.$testing_list['ServiceDeviceInfo']['id'];?>'  <?php echo $class;?>>
		
         
		<td align='left'><div style='width: 100px;' class='alistname'><?php echo $testing_list['ServiceDeviceInfo']['recive_date']; ?>&nbsp;</div></td>
		<td align='left'><div style='width: 100px;' class='alistname'><?php echo $testing_list['ServiceDeviceInfo']['estimated_date']; ?>&nbsp;</div></td>
        
        
        <td align='left'>
        <div style='width: 90px;' class='alistname'><?php 
		 if($testing_list['ServiceDeviceInfo']['status']==1){

            echo "<div class='link_view' id='Receive'>Receive</div>"; 
		
          }else if($testing_list['ServiceDeviceInfo']['status']==2 ){
		
		    echo "<div class='link_view' id='wait_approve'>Assessment</div>"; 
		  }else if($testing_list['ServiceDeviceInfo']['status']==3 ){
		
		    echo "<div class='link_view' id='reassessment'>Re-Assessment</div>"; 
		  }else if($testing_list['ServiceDeviceInfo']['status']==4 ){
		
		    echo "<div class='link_view' id='Approved'>Confirmation</div>"; 
		  }else if($testing_list['ServiceDeviceInfo']['status']==5 ){
		
		    echo "<div class='link_view' id='servicing'>Servicing</div>"; 
		  }
		   else if($testing_list['ServiceDeviceInfo']['status']==6 ){
		
		    echo "<div class='link_view' id='s_complete'>Service Complete</div>"; 
		  }
		  else if($testing_list['ServiceDeviceInfo']['status']==7 ){
		
		    echo "<div class='link_view' id='Testing'>Testing</div>"; 
		  }
		  else if($testing_list['ServiceDeviceInfo']['status']==8 ){
		
		    echo "<div class='link_view'>Wait for delivery</div>"; 
		  }
		  else if($testing_list['ServiceDeviceInfo']['status']==9 ){
		
		    echo "<div class='link_view'>Delivered</div>"; 
		  }
		...
		<td align='left'><div style='width: 100px;' class='alistname'><?php echo $testing_list['PosCustomer']['name']; ?>&nbsp;</div></td>
		<td align='left'><div style='width: 100px;' class='alistname'><?php echo $testing_list['ServiceDevice']['name']; ?>&nbsp;</div></td>
        <td align='left'><div style='width: 100px;' class='alistname'><?php echo $testing_list['ServiceDeviceInfo']['serial_no']; ?>&nbsp;</div></td>
        <td align='left'><div style='width: 60px;' class='alistname'><?php echo $testing_list['ServiceInvoice']['inventory_total']; ?>&nbsp;</div></td>
        <td align='left'><div style='width: 60px;' class='alistname'><?php echo $testing_list['ServiceInvoice']['service_total']; ?>&nbsp;</div></td>
        
		
		
		<?php /*?><td class="actions">
<?php if($testing_list['ServiceDeviceInfo']['status']==6){?>
<div style='width: 180px;' class='alistname link_link'>
<?php echo $this->Html->link(__('Testing', true), array('controller'=>'ServiceDeviceInfos','action' =>'testing_list'),array('class'=>'link_view action_link','id'=>'Testing')); ?>
		</div>
   <?php }else if($testing_list['ServiceDeviceInfo']['status']==7){?>
	   
<?php echo $this->Html->link(__('Tested', true), array('controller'=>'ServiceDeviceInfos','action' =>'testing_list'),array('class'=>'link_view action_link','id'=>'Tested')); ?>
