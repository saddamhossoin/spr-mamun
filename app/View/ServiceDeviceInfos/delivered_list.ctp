...
<?php
	...
	<tr id='<?php echo 'row_'.$delivered_list['ServiceDeviceInfo']['id'];?>'  <?php echo $class;?>>
		...
		<td class="actions">
        <?php if($delivered_list['ServiceDeviceInfo']['status']==9){?>
<div style='width: 180px;' class='alistname link_link'>
 <?php echo $this->Html->link(__('Delivered Invoice', true), array('controller'=>'ServiceDeviceInfos','action' =>'delivered_list'),array('class'=>'link_view action_link')); ?>
		</div>
        <?php }?>
