...
<div>
        ...
        <ul id="brands" class="product_brand">
		  <?php
            ...
            foreach ($brandlists as $posBrand){?>
             <li>
                <?php  if($posBrand['PosBrand']['slug'] == 'tools' ){
					 echo $this->Html->link($posBrand['PosBrand']['name'], array('controller'=>'PosProducts','action'=>'device_accessories',$posBrand['PosBrand']['slug']));
				 }else{
    ...
    <div class="rightpan">
    	<div class="header"><?php echo $type_products['PosProduct']['name'];?></div>
        	<div id="img_parent">
				...
				<?php if(!empty($type_products['PosProduct']['image'])) {
                 echo $this->Html->image('../'.$type_products['PosProduct']['image'], array("alt" => $type_products['PosProduct']['name'],"width"=>"78" ,"height"=>"58","border"=>"0",'url' => array('controller' => 'PosProducts', 'action' =>'device_accessories',$type_products['PosProduct']['slug']))); }else{ echo $this->Html->image('/img/wpage/noimage.jpg', array("alt" => "No Image","width"=>"78" ,"height"=>"58","border"=>"0"));}?>
     ...
     <?php  }else{ 
      			 echo "<h3 class='product_msg'>There are no products available.</h3>"; 
     		  } ?>
 		</div>
                    ...
                    // pr($compitableProductItem);
                     if(!empty($compitableProductItem)){?>
                           <?php foreach($compitableProductItem as $comProduct){  ?>      
                             ...
                             <div class="product_name_main"><?php echo $this->Html->link($comProduct['PosProduct']['name'], array('controller'=>'PosProducts','action'=>'details',$comProduct['PosProduct']['slug']));?></div>
                               ...
                               <div class="description_grid">
                                <p>
								 ...
								 //  pr($comProduct);
									   ...
									   echo $comProduct['PosProduct']['online_price']."<span> + iva</span>";
                             ...
                             <?php }else{?>
                               ...
                               <div class="description_grid">
                                <p>
                                 ...
                                 <div class="price_qty_div">
                                   <span class="price_span pricetxt"  > &#8364; <?php  echo $comProduct['PosProduct']['online_price']."<span> + iva</span>";?></span>                                 <span class="price_span">
 							  <?php echo $this->Form->create(NULL, array('url' => array('controller' => 'shop', 'action' => 'add'),'id'=>'ShopCard_'.$comProduct['PosProduct']['id'])); ?>
                        <?php echo $this->Form->input('id', array('type' => 'hidden', 'value' => $comProduct['PosProduct']['id'])); ?>
                                    <input type='button' value='-' class='qtyminus' id="btnqtyminus_<?php echo $comProduct['PosProduct']['id'];?>" field='quandtity' />
                                    <input type='text' name='data[PosProduct][quantity]' value=<?php echo $cartval;?>  id="qtyplus_<?php echo $comProduct['PosProduct']['id'];?>" class='qty' />
