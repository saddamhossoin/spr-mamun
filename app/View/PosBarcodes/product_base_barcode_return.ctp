...
//========================== Start Take Inventory ============================
	...
	$(".popup_add_link").on('click',function(e){
		...
		var p_id_for_b = $("#PosBarcodePosSaleDetailId").val();
	    ...
	    var barcode_val= $("#PosBarcodeBarcode_"+id[1]).val();	
			...
			if(input_field_check() <= (barcode_quantity-1)){
					 ...
					 $(".barcode_"+p_id_for_b).append("<input type='hidden' value='"+barcode_val+"' id='PosBarcodeBarcode_"+serial_b+"' name='data[PosSaleReturnDetail]["+Product_id+"][PosBarcode]["+serial_b+"]'>");
...
function input_field_check(){
		var product_id_barocde = $("#PosBarcodePosSaleDetailId").val();
