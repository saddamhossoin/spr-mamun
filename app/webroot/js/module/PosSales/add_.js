jQuery(function($){
	...
	//======================= Start Add Script =====================				
 	 $('#btn_PosSale_add').click(function(){
 	 	...
 	 	//========================== Validation Check ========
  	if( $('#PosSaleAddForm').valid() && $('#PosSaleAmountAddForm').valid() && $('.productlist').children().length > 0) 
 		{
 		...
 		if( quantities >0  &&    stock_Sales_Price <= entry_sales_price){
			...
			if(!is_barcode){
				...
				if( productid ){
					...
					//	if($(this).attr('id') == productid) id_8088
					//var lineId = $(this).attr('id');
//					var is_have = $("#id_"+productid).;
					if($("#id_"+productid).length === 0)
					{
					 	$.ajax({
							...
							$(".productlistli_"+productid).addClass('haveRow');
							$.alert.open({
								type: 'confirm',
								content: 'This product '+$("#id_"+productid).val()+' item already taken, are you update the quantity!!!',
								callback: function(button) {
									if (button == 'yes'){
										$(".productlistli_"+productid).remove();
							  			var is_not_same = 1;
...
//========================product status=============================//
 ...
 //============== Product Search By Barcode ==========================
	...
	$("#PosProductName").on("keyup", function(e) {
		 if($(this).val().length >9 && $(this).val().substring(0,3).toLowerCase() == 'spr'){
			$.ajax({
					type: "GET",
 ...
 var sl_no = 0;	
	...
	function checkDuplicate(){
		...
		$('.productlist tr').each(function(index) {
			 ...
			 {
				var lineId = $(this).attr('id');
				$(".productlistli_"+lineId).addClass('haveRow');
				$.alert.open({
					type: 'confirm',
					content: 'This product '+$("#id_"+lineId).val()+' item already taken, are you update the quantity!!!',
					callback: function(button) {
						if (button == 'yes'){
 	...
 	function addtogrid( object ,  Quantity  ){
		
		//
		var productid= $("#product_id").html();
		var is_barcode= $("#is_barcode").html();
		var product_type_id = object.PosProduct.pos_type_id;
...
//=============================== For Serial and alterrow =====================	
	...
	$(".productlist").append("<tr  class='productlistli_"+productid+" "+classalt+"' id='"+productid+"'><td>"+sl_no+"</td><td><input type='hidden' class=' productid' value="+productid+" name='data[PosSaleDetail]["+productid+"][pos_product_id]'><span class='productvalli'>"+productval+"</span><input type='hidden' class='brandid' value="+brandid+" name='data[PosSaleDetail]["+productid+"][pos_brand_id]'></td><td><span class='brandvalli'>"+brandval+"</span><input type='hidden' class='categoryid' value="+categoryid+" name='data[PosSaleDetail]["+productid+"][pos_pcategory_id]'></td><td><span class='categoryvalli'>"+categoryval+"</span></td><td><span class='Quantityli'>"+Quantity+"</span><input type='hidden' class='categoryid' id='id_"+productid+"' value="+Quantity+" name='data[PosSaleDetail]["+productid+"][quantity]'></td><td><span class='purchasepriseli'>"+purchaseprise+"</span><input type='hidden' class='categoryid' value="+purchaseprise+" name='data[PosSaleDetail]["+productid+"][price]'></td><td><span class='totalpriceli'>"+totalprice+"</span><input type='hidden' class='categoryid' value="+totalprice+" name='data[PosSaleDetail]["+productid+"][totalprice]'><span id='all_barcode_sale' class='"+barcode_class+"'></span></td><td><span class='deletelink' id='deletelink_"+productid+"'></span></td></tr>");
