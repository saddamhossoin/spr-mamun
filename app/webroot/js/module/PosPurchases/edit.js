jQuery(function($){ 
 	 ...
 	 $('#btn_PosPurchase_update').click(function(){
 		...
 		//========================== Validation Check ========
  		if( $('#PosPurchaseEditForm').valid() && $('#PosPurchaseAmountEditForm').valid()) 
 		{  
			$('.ajax_status').fadeIn();
  ...
  var data= $('#PosPurchaseEditForm').serialize() + '&' + $('#PosPurchaseAmountEditForm').serialize()+ '&' +$('#PosPurchaseDetailEditForm').serialize();
  	//=================================//
		$.ajax({
			type: "POST",
			url:siteurl+"PosPurchases/edit/"+$('#PosPurchaseId').val(),
			data: data,
...
//===================== Auto Complete Calling ====================
	...
	{
			$("#PosPurchaseTotalprice").val("0.00");
